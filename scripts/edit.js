document.addEventListener('DOMContentLoaded', () => {
    ...
    chrome.storage.local.get(['deck'], result => {
        ...
        Object.keys(cardList).forEach(key => {
            ...
            newListItem.classList.add('mt-3', 'fcard');
            newListItem.id = key;

            // element for word
            const wordElement = document.createElement('p');
            wordElement.textContent = card.word;
            newListItem.appendChild(wordElement);
            ...
            newListItem.appendChild(translationElement);
            ...
            deleteButton.addEventListener('click', () => {
                cardListElement.removeChild(newListItem);
                deck.size--;
